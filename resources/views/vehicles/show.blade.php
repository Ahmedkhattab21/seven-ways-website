...
@section('content')
<div class="vehicle-profile-layout">
<x-card title="بيانات السيارة"><dl class="sw-details-grid vehicle-profile-details">
...
@can('transfer',$vehicle)<x-card title="نقل الملكية"><form method="POST" action="{{ route('vehicles.transfer',$vehicle) }}" class="sw-form vehicle-transfer-form">@csrf<div class="sw-form-grid"><x-form.select name="customer_id" label="العميل الجديد" required>@foreach($customers as $customer)<option value="{{ $customer->id }}">{{ $customer->customer_code }} — {{ $customer->name }}</option>@endforeach</x-form.select><x-form.input name="transferred_at" type="datetime-local" label="تاريخ النقل" /><x-form.input name="reason" label="السبب" /><x-form.textarea name="notes" label="ملاحظات"></x-form.textarea></div><div class="sw-form-actions"><x-button type="submit">نقل الملكية</x-button></div></form></x-card>@endcan
