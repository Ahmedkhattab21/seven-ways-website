...
<x-table-shell>
    ...
    <tbody>@foreach($roles as $role)<tr><td>{{ $role->display_name }}</td><td>{{ $role->scope }}</td><td>{{ $role->users_count }}</td><td>{{ $role->permissions_count }}</td><td>@if(!$role->is_system && auth()->user()->hasPermission('roles.manage'))<a href="{{ route('roles.edit', $role) }}">إدارة الصلاحيات</a>@else<span>مرجعي</span>@endif</td></tr>@endforeach</tbody>
</x-table-shell>
@endsection
