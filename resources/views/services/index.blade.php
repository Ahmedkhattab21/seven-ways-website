...
<x-card title="البحث والفلاتر"><form method="GET" class="sw-form"><div class="sw-form-grid">
    ...
    <x-form.select name="branch_id" label="الفرع"><option value="">الكل</option>@foreach($branches as $branch)<option value="{{ $branch->id }}" @selected(request('branch_id')==$branch->id)>{{ $branch->name }}</option>@endforeach</x-form.select>
...
<x-table-shell>
<thead><tr><th>الكود</th><th>الخدمة</th><th>التصنيف</th><th>النوع</th><th>التسعير</th><th>الفروع المتاحة</th><th>الحالة</th><th></th></tr></thead>
<tbody>@forelse($services as $service)<tr>
    <td>{{ $service->code }}</td><td><a href="{{ route('services.show', $service) }}">{{ $service->name }}</a></td>
    <td>{{ $service->category?->name }}</td><td>{{ $service->service_type }}</td><td>{{ $service->pricing_type }}</td>
    <td>{{ $service->branchServices->where('is_available', true)->count() }}</td><td><x-status-badge :status="$service->is_active ? 'active' : 'inactive'" /></td>
    <td>@if(auth()->user()->hasPermission('services.update'))<a href="{{ route('services.edit', $service) }}">تعديل</a>@endif</td>
</tr>@empty<tr><td colspan="8">لا توجد خدمات.</td></tr>@endforelse</tbody>
