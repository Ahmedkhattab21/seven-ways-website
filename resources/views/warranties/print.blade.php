<!doctype html>
<html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>{{ $warranty->warranty_number }}</title><style>body{font-family:Arial;margin:40px;color:#111}.brand{color:#b5121b;font-size:28px;font-weight:bold}table{width:100%;border-collapse:collapse}td,th{border:1px solid #bbb;padding:8px}.state{font-size:24px;font-weight:bold}.void{color:#b5121b}@media print{button{display:none}}</style></head>
<body>
<div class="brand">SEVEN WAYS</div><h1>شهادة ضمان</h1><p class="state {{ in_array($warranty->status, ['void', 'expired']) ? 'void' : '' }}">{{ strtoupper($warranty->status) }}</p>
