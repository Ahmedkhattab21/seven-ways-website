...
@php($statusLabels = ['opened' => 'مفتوحة', 'counting' => 'جارٍ الجرد', 'pending_approval' => 'بانتظار الاعتماد', 'approved' => 'معتمدة', 'closed' => 'مغلقة'])
...
@endif
@foreach($session->counts as $count)<div><p>{{ $countTypes[$count->count_type] ?? $count->count_type }}: {{ $count->counted_total }} | الرصيد الدفتري {{ $count->book_total }} | الفرق {{ $count->difference }} | {{ $count->status }}</p>@php($countActions = ['draft'=>['submit','count','إرسال العد للمراجعة'],'submitted'=>['review','review','مراجعة العد'],'reviewed'=>['approve','approve','اعتماد العد']])@if($countAction = $countActions[$count->status] ?? null)@if(auth()->user()->hasPermission('treasury.cash_sessions.'.$countAction[1]))<form method="POST" action="{{ route('treasury.cash-counts.action', [$count, $countAction[0]]) }}" style="display:inline">@csrf<button class="sw-btn">{{ $countAction[2] }}</button></form>@endif @endif</div>@endforeach
@php($approvedClosing = $session->counts->contains(fn ($count) => $count->count_type === 'closing' && $count->status === 'approved'))
<p>حالة الجلسة: {{ $statusLabels[$session->status] ?? $session->status }}</p>
@if(in_array($session->status, ['counting', 'approved'], true) && ! $approvedClosing)<p class="sw-alert sw-alert--warning">يجب تسجيل ومراجعة واعتماد العد الختامي أولاً قبل إرسال الجلسة أو إغلاقها.</p>@endif
@php($sessionActions = ['opened'=>['start_counting','count'],'counting'=>['submit','submit'],'pending_approval'=>['approve','approve'],'approved'=>['close','close'],'closed'=>['reopen','reopen']])@if($sessionAction = $sessionActions[$session->status] ?? null)@if($approvedClosing || ! in_array($session->status, ['counting','pending_approval','approved'], true))@if(auth()->user()->hasPermission('treasury.cash_sessions.'.$sessionAction[1]))<form method="POST" action="{{ route('treasury.cash-sessions.action', [$session, $sessionAction[0]]) }}" style="display:inline">@csrf<button class="sw-btn">{{ $session->status === 'approved' ? 'إغلاق الجلسة' : ($session->status === 'counting' ? 'إرسال الجلسة للاعتماد' : $sessionAction[0]) }}</button></form>@endif @endif @endif
</section>
@endforeach
{{ $sessions->links() }}
